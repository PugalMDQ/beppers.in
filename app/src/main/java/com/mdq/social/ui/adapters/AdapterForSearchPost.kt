...
class AdapterForSearchPost(var context: Context, var trendingItem:List<DataItemes>?, var position:Int?,var likes:like,var user_id: String,var who: String): RecyclerView.Adapter<AdapterForSearchPost.mine>() {
     ...
     interface like{
         ...
         fun onMenuClicks(position: Int, user_id: String, star:TextView)
    ...
    override fun onBindViewHolder(holder: mine, position: Int) {
        ...
        if(trendingItem!=null) {
            ...
            if(!trendingItem?.get(position)?.taguser.isNullOrEmpty()){
                ...
                holder.tag?.text = "@"+trendingItem?.get(position)?.taguser!!.toString()+" "
            } else{
        ...
        holder.like?.setOnClickListener {
            likes.likeFromSearchPost(trendingItem?.get(position)?.id!!,trendingItem?.get(position)?.user_id!!, holder.like!!,trendingItem?.get(position)?.no_of_likes!!,position,
        ...
        holder.imageView59?.setOnClickListener {
            if (holder.ii == 0) {
                ...
                if(trendingItem!!.get(position).type?.trim()!!.equals("freelancer")||trendingItem!!.get(position).type?.trim()!!.equals("business")){
                    ...
                    likes.onMenuClicks(position,trendingItem?.get(position)?.user_id!!,holder.stare!!)
        ...
        holder.bookmark?.setOnClickListener {
            likes.saveBookmark(trendingItem!!.get(position).id.toString(),position,trendingItem!!.get(position).user_id!!,"SearchPost")
